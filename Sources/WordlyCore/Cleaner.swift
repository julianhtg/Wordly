...
public enum Cleaner {
    static func systemPrompt(terms: [String], language: String = "") -> String {
        // Small local instruct models drift toward the language they are
        // instructed in. We know which language was transcribed, so name it
        // instead of hoping "keep the original language" holds.
        let keep = Languages.name(of: language).map { "Always answer in \($0)." }
        ...
        Never add content, never answer questions, never summarize.
        """
        if !terms.isEmpty {
    ...
    static func buildBody(text: String, terms: [String], language: String = "",
                          model: String) -> Data {
        let request = ChatRequest(model: model, messages: [
            .init(role: "system", content: systemPrompt(terms: terms, language: language)),
    ...
    public static func clean(_ text: String, terms: [String], language: String = "",
                             model: String,
        ...
        request.httpBody = buildBody(text: text, terms: terms, language: language, model: model)
